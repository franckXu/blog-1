...
I will start off with a word of caution that singletons should be avoided.
...
some component that exposes a service to other components.

Implementation
--------------

`This CppCon lightning talk <https://www.youtube.com/watch?v=23xDn3ReH7E>`_
...
I completely agree with this, with the caveat that sometimes we do want to
...
Given an interface, an implementation, and a function to retrieve the singleton
like the following::
...
If the goal is to inject the dependency, for example have tests run against
``MockFoo``, this approach is not ideal. Code explicitly calls ``UseFoo`` so the
only way to switch implementation is to modify ``UseFoo`` and provide some
internal mechanism to change its return value. A better approach is to have the
client simply require an interface and provide that at construction time::
...
Note that in the above example we can create ``Component`` with a ``MockFoo``
implementation of ``IFoo`` or some other implementation, which is a better
decoupling than directly calling ``UseFoo`` inside the member functions of
``Component``.

Magic Statics
-------------

By definition, a singleton should represent a unique object, so our ``UseFoo``
...
introduced “magic statics”. Quote from the C++ standard section 6.7:

    ... such a variable [with static storage] is initialized the first time
...
The standard now guarantees that a static would only ever be created once, and
the simple way to implement a singleton (for example according to Scott Meyer’s
`Effective Modern C++ <https://www.goodreads.com/book/show/22800553-effective-modern-c>`_)
is::
...
A more interesting problem which the above implementation doesn’t cover is
...
Using this implementation, we can deterministically free the singleton on demand
via the ``Free`` function as opposed to having to wait for the program to get
unloaded, which can be useful in certain cases.

Atomics
-------

Magic statics provide an easy way to guarantee we end up with a single object,
but the code generated to support this is non-trivial. Disassembly of the
``UseFoo`` above as compiled by Clang 4.0.0 with ``-O3`` flag:
...
A lot of the generated code is the compiler implementing the guarantee that on
concurrent calls, a single intialization is performed. The ``Singleton``
functions are inlined here since we are compiling with ``-O3``. We can provide a
...
This code might new the object multiple times, but is guaranteed to always
...
to my colleague Vladimir Morozov who suggested this approach.
...
We now have an efficient generic singleton which we can shutdown on-demand and
ensure clients never call after shutdown.
...
* If dependency injection is required, make sure dependency is properly injected
  during construction as opposed to member functions directly calling the
  singleton-retrieving function
* Magic statics provide an easy way to implement singletons
* Atomics are more efficient than magic statics when they are lock-free and we
